...
export const addAnnotations = (
	...
	const annotations = blockProblems.filter(
		(problem) => problem.state !== 'ignored' && !ignore.includes(problem.id)
	);
	...
	const ANNOTATION_LIST = [
		...
		...annotations.filter(
			(annotation) => annotation.type && annotation.type === 'assuming'
		),
		...annotations.filter(
...
export const addBlockToQueue = (block, unshift) => {
	if (block) {
		...
		queue = queue.filter(
			(queuedBlock) => block.clientId !== queuedBlock.clientId
		);
...
export const parseBlockText = async (
	text = '',
	{ preserveWhiteSpace = true, offset = 0 }
) => {
	if (!text) {
		return {
			nodes: [],
	...
	const { writers_blocks } = select('core').getEntityRecord(
		...
		writers_blocks:
			DEFAULT_SETTINGS.demo !== true
				? DEFAULT_SETTINGS
				: JSON.parse(window.localStorage.getItem('writers_blocks')),
	};
	const {
		dictionary = '',
...
export const getAnnotatableTextFromBlock = async (block) => {
	const { name: blockName, attributes: blockAttributes, blockId } = block;
	...
	const { nodes, messages: problems } = await parseBlockText(
	...
	console.log(
		`${blockName}: ${
			window.performance.getEntriesByName(`${blockName}-${id}-time`)[0]
				.duration
		}ms`
	);
...
export const getAnnotatableText = (blocks) => {
	...
	const { nodes, problems } = allowedBlocks.reduce(
		async (acc, block) => {
			const { nodes: blockNodes, problems: blockProblems } =
...
export const scheduleAnnotations = debounce(async () => {
	...
	const { problems: blockProblems, nodes: blockNodes } =
		await getAnnotatableTextFromBlock(block);
	...
	if (blockProblems.length) {
		...
		const filteredProblems = problems.filter(
			(problem) => problem.blockId !== clientId
		);
	...
	const {
		mode: writingMode = DEFAULT_SETTINGS.demo === true
			? 'editing'
			: 'writing',
	} = writers_blocks;
