				...
				if (response) {
					setSelectedWordData({
						...
						results: (response.results || []).reduce(
							(acc, result) => ({
								...acc,
								[result.partOfSpeech]: acc[result.partOfSpeech]
									? [...acc[result.partOfSpeech], result]
									: [result],
							}),
							{}
						),
		...
		<Fragment>
			<ToolbarButton
				...
				onClick={() => {
					...
					const { offset: selectionEnd, attributeKey: content } =
					...
					if (selectionStart !== selectionEnd) {
						...
						if (word.includes(' ')) setIsConfirmModalOpen(true);
						else if (word) {
			...
			{isConfirmModalOpen ? (
				<Confirm
					...
					message={__(
						`It looks like you've selected multiple words. Are you sure you want to use a search credit?`,
						'writers-blocks'
					)}
				/>
			) : null}
				...
				>
					{isSearchModalLoading ? (
						...
						<div className="wp-block-writers-blocks-word__modal-content">
							<Flex
								className="wp-block-writers-blocks-word__modal-title"
								justify="flex-start"
							>
								<h2>{selectedWord}</h2>
								<small>
									{selectedWordData?.pronunciation?.all}
								</small>
							</Flex>
							{selectedWordData &&
							Object.keys(selectedWordData.results).length ? (
								<div>
									{Object.keys(selectedWordData.results).map(
										(key, index) => {
											return (
												<Fragment key={index}>
								...
								<p>
									{__('No results found', 'writers-blocks')}
								</p>
							)}
						</div>
					)}
