...
const { WRITERS_BLOCKS: { rest_url, license: LICENSE_DATA } } = window;

export const LicenseChecker = () => {
    const [licenseData, setLicenseData] = useState( LICENSE_DATA );
    const [license, setLicense] = useState( LICENSE_DATA?.license_key?.key || '' );
    ...
    const verifyLicense = async () => {
        ...
        const action = licenseData && licenseData.activated || licenseData.valid
            ? 'validate'
            : 'activate';
    ...
    return (
        <Fragment>
            <HStack alignment='stretch'>
                ...
                >
                    { __( ! isLicenseValid ? 'Activate' : 'Validate', 'writers-blocks' ) }
                </Button>
            </HStack>
            { isLicenseUpdating ? (
                <Spinner />
            ) : null }
            { licenseData && ( isLicenseValid || licenseData.error || licenseData?.license_key?.key === license ) && ! isLicenseUpdating ? (
                <p className={ `writers-blocks__license-checker-message${ isLicenseValid ? ' is-valid' : ' is-invalid' }` }>
                    { <Icon icon={ isLicenseValid ? check : close } size="16" /> }
                    { __( isLicenseValid ? 'License is valid' : 'Invalid license', 'writers-blocks' ) }
                </p>
            ) : null }
        </Fragment>
    );
