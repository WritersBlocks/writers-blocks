...
const flattenBlocks = (blocks) => {
	return blocks.reduce((accumulator, block) => {
		if (block.innerBlocks.length) {
			return [...accumulator, ...flattenBlocks(block.innerBlocks)];
		}
...
const getBlockContent = (block) =>
	block.attributes[BLOCK_TYPE_CONTENT_ATTRIBUTE[block.name] ?? 'content'] ??
	'';

const hasContentChanged = (blockContent) =>
	JSON.stringify(blockContent) !== JSON.stringify(_blocks);
...
domReady(() => {
	subscribe(
		debounce(() => {
			...
			const blockContent = flattenBlocks(blocks)
				.filter(isBlockAllowed)
			...
			if (selectedBlock) {
				...
				if (
					!_blocks[selectedBlock.clientId] ||
					selectedBlockContent !== _blocks[selectedBlock.clientId]
				) {
